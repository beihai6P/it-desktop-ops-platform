...
import { Search, Filter, Plus, TrendingUp, Download, Flame, Lock } from 'lucide-react';
...
import { apiPost, apiDelete } from '@/scheduler';
...
export default function Tools() {
  ...
  const handleFeature = async (id: string) => {
    try {
      await apiPost('/tools/' + id + '/feature');
      await loadTools();
    } catch (error) {
  ...
  const categories = ['全部', '基础工具', '系统工具', '网络工具', '云工具', '工具'];
        ...
        <div className="bg-gradient-to-br from-blue-100 to-blue-50 rounded-xl p-5 border border-blue-200">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-sm text-text-muted">已认证工具</p>
            </div>
          </div>
        </div>
      </div>
